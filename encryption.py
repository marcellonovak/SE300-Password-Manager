...
def gen_file(password):
    ...
    file.write(
        base64.b64encode(hashlib.sha512(password.encode("utf-8")).digest()).decode(
            "utf-8"
        )
    )
    file.write("\n")
...
def read_services(password):
    ...
    if (
        ...
        servs = (
            Fernet(key)
            .decrypt(pwfile[2][:-1].encode("utf-8"))
            .decode("utf-8")
            .split(",")
        )
        info = (
            Fernet(key)
            .decrypt(pwfile[3][:-1].encode("utf-8"))
            .decode("utf-8")
            .split("\t\n\t")
        )
...
def add_service(password, service, serv_name, serv_pass, info="\n"):
    ...
    if (
        ...
        key = base64.b64encode(hashlib.sha256(password.encode("utf-8")).digest())
        serv_count = int(
            Fernet(key).decrypt(pwfile[1][:-1].encode("utf-8")).decode("utf-8")
        )
        servs = (
            Fernet(key)
            .decrypt(pwfile[2][:-1].encode("utf-8"))
        ...
        infos.append(info)
        pwfile.append(
            f"{base64.b64encode(hashlib.sha512(service.encode('utf-8')).digest()).decode('utf-8')}\n"
        )
        pwfile.append(
            f"{Fernet(key).encrypt(serv_name.encode('utf-8')).decode('utf-8')}\n"
        )
        pwfile.append(
            f"{Fernet(key).encrypt(serv_pass.encode('utf-8')).decode('utf-8')}\n"
        )
        servs_txt = ",".join(servs)
        info_txt = "\t\n\t".join(infos)
        pwfile[1] = (
            f"{Fernet(key).encrypt(str(len(servs)).encode('utf-8')).decode('utf-8')}\n"
        )
        pwfile[2] = (
            f"{Fernet(key).encrypt(servs_txt.encode('utf-8')).decode('utf-8')}\n"
        )
        ...
        for i in range(1, len(servs)):
            if servs[i] == service:
                dat_ent = [
                    service,
                    Fernet(key)
                    .decrypt(pwfile[1 + i * 3 + 1][:-1].encode("utf-8"))
                    .decode("utf-8"),
                    Fernet(key)
                    .decrypt(pwfile[1 + i * 3 + 2][:-1].encode("utf-8"))
                    .decode("utf-8"),
                ]
                data.append(dat_ent)
        return data
...
def read_data_by_ID(password, id):
    ...
    if (
        ...
        dat_ent = [
            servs[id + 1],
            Fernet(key)
            .decrypt(pwfile[4 + id * 3 + 1][:-1].encode("utf-8"))
            .decode("utf-8"),
            Fernet(key)
            .decrypt(pwfile[4 + id * 3 + 2][:-1].encode("utf-8"))
            .decode("utf-8"),
        ]
        return dat_ent
